...
pg.display.set_caption("minesweeper (server)")
...
def connect():
    ...
    while True:
        try:
            ...
            other_socket = server_socket.accept()[0]
            break
        except :
            pass

def update_data():
    global other_main, other_state, connecting, other_bomb, other_die, other_win, other_red_block
    connecting = True
    while connecting:
        try:
            data = json.loads(other_socket.recv(1024).decode())
            ...
            other_socket.sendall(send_data.encode())
        except:
            connecting = False
